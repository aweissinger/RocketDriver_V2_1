...
Bmi088Accel::Bmi088Accel(SPIClass &bus)
{
  _spi = &bus; // SPI bus
...
int Bmi088Accel::begin()
{
  if (_useSPI) {
    ...
    pinMode(BMI085_ACCEL_CS_PIN,OUTPUT);
    digitalWrite(BMI085_ACCEL_CS_PIN,HIGH);
...
void Bmi088Accel::softReset()
{
  ...
  delay(50);
  if (_useSPI) {
    digitalWrite(BMI085_ACCEL_CS_PIN,LOW);
    digitalWrite(BMI085_ACCEL_CS_PIN,HIGH);
  }
}
...
void Bmi088Accel::writeRegister(uint8_t subAddress, uint8_t data)
{
  if (_useSPI) {
    ...
    digitalWrite(BMI085_ACCEL_CS_PIN,LOW); // select the chip
    _spi->transfer(subAddress & ~SPI_READ); // write the register address
    _spi->transfer(data); // write the data
    digitalWrite(BMI085_ACCEL_CS_PIN,HIGH); // deselect the chip
...
/* BMI088 object, input the SPI bus and chip select pin */
Bmi088Gyro::Bmi088Gyro(SPIClass &bus)
{
  _spi = &bus; // SPI bus
  _useSPI = true; // set to use SPI
}
...
int Bmi088Gyro::begin()
{
  if (_useSPI) {
    // setting CS pin to output
    pinMode(BMI085_GYRO_CS_PIN,OUTPUT);
    // setting CS pin high
    digitalWrite(BMI085_GYRO_CS_PIN,HIGH);
    // begin SPI communication
    _spi->begin();
  } else {
    /* starting the I2C bus */
...
void Bmi088Gyro::writeRegister(uint8_t subAddress, uint8_t data)
{
  if (_useSPI) {
    _spi->beginTransaction(SPISettings(SPI_CLOCK, MSBFIRST, SPI_MODE0)); // begin the transaction
    digitalWrite(BMI085_GYRO_CS_PIN,LOW); // select the chip
    _spi->transfer(subAddress & ~SPI_READ); // write the register address
    _spi->transfer(data); // write the data
    digitalWrite(BMI085_GYRO_CS_PIN,HIGH); // deselect the chip
    _spi->endTransaction(); // end the transaction
  } else {
    _i2c->beginTransmission(_address); // open the device
    _i2c->write(subAddress); // write the register address
...
/* BMI088 object, input the SPI bus and chip select pin */
Bmi088::Bmi088(SPIClass &bus)
{
  accel = new Bmi088Accel(bus);
  gyro = new Bmi088Gyro(bus);
}
